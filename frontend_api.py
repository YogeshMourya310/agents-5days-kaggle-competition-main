...
app = FastAPI(
    ...
    description="Multi-Agent NSE Stock Prediction System with A2A-compatible components",
    version="1.0.0"
)
...
Assume the stock is an Indian equity, so explain things in an NSE and INR context where relevant. Write in clear, conversational language. Avoid jargon. Use analogies when helpful. Keep it concise (2-3 paragraphs). Focus on what matters to an investor.
"""
...
async def generate_investor_advice(request: dict):
    ...
    try:
        ...
        # Create Gemini prompt
        prompt = f"""You are an experienced investment advisor. Based on the multi-agent AI analysis below, provide clear, actionable advice for a retail investor analyzing an Indian stock.
...
Use an NSE and INR context where relevant. Write in plain English, avoiding jargon. Be balanced and mention both positives and negatives. Keep it concise (3-4 paragraphs).
"""
        ...
        if macro_data.get('directional_signal', 0) != 0:
            ...
            repo_rate = macro_metrics.get('repo_rate', macro_metrics.get('fed_funds_rate', 'N/A'))
            insights.append(f"- **Economic environment**: {regime.title()} regime with RBI policy rate at {repo_rate}")
...
*This analysis synthesizes NSE-oriented market data, India macro inputs, public news flow, and company announcement signals through our multi-agent architecture.*"""
