...
function TradingViewWidget({ ticker, theme = 'dark' }: TradingViewWidgetProps) {
  ...
  const normalizedSymbol = ticker.includes('.') ? ticker : `${ticker}.NS`;
  const tradingViewSymbol = `NSE:${ticker.replace('.NS', '')}`;
  ...
  }, [ticker, theme, tradingViewSymbol]);
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h3 className="text-xl font-bold text-white">NSE Chart View</h3>
          </div>
          <p className="text-slate-400 text-sm">
            Live market context for <span className="text-cyan-400 font-bold">{normalizedSymbol}</span>
          </p>
        </div>
